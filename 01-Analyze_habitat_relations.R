...
parameters <- c("omega", "rho.ab", "rho.bd",
                ...
                "bd.PACC10_3km", "bd.mnPtchAr_Gap3km", "bd.mnPerArRatio_Gap3km", "bd.NNdist_Gap3km",
                "bd.PACC40_3km", "bd.mnPtchAr_Opn3km", "bd.mnPerArRatio_Opn3km", "bd.NNdist_Opn3km",
                "bd.TWIP", "bd.Rdens",
                "bb.CanCov", "bb.CanHt", "bb.NumSnags", "bb.RCOV_PP", "bb.RCOV_DF",
                "bb.RCOV_AS", "bb.RSCV_Ladder", "bb.RSCV_Ber", "bb.HerbGrassVol", "bb.SOHtRatio",
                "ba.Time", "ba.Time2", "ba.DOY", "ba.ccov", "ba.shvol",
...
inits <- function()
  list(z=z.init, u=u.init, w=w.init, tvar.sigma.a0 = rnorm(1), tvar.sigma.b0 = rnorm(1), tvar.sigma.d0 = rnorm(n.year),
       ...
       tvar.Betab.HerbGrassVol = rnorm(1), tvar.Betab.SOHtRatio = rnorm(1),
...
PACC10_3km.d <- landscape_data$PACC10_3km %>% (function(x) (x - mean(x, na.rm = T)) / sd(x, na.rm = T))

mnPtchAr_Gap3km.d <- landscape_data$mnPtchAr_Gap3km %>% (function(x) (x - mean(x, na.rm = T)) / sd(x, na.rm = T))
mnPtchAr_Gap3km.d[which(is.na(mnPtchAr_Gap3km.d))] <- 0

mnPerArRatio_Gap3km.d <- landscape_data$mnPerArRatio_Gap3km %>% (function(x) (x - mean(x, na.rm = T)) / sd(x, na.rm = T))
...
NNdist_Gap3km.d <- landscape_data$NNdist_Gap3km %>% (function(x) (x - mean(x, na.rm = T)) / sd(x, na.rm = T))
...
PACC40_3km.d <- landscape_data$PACC40_3km %>% (function(x) (x - mean(x, na.rm = T)) / sd(x, na.rm = T))

mnPtchAr_Opn3km.d <- landscape_data$mnPtchAr_Opn3km %>% (function(x) (x - mean(x, na.rm = T)) / sd(x, na.rm = T))
mnPtchAr_Opn3km.d[which(is.na(mnPtchAr_Opn3km.d))] <- 0

mnPerArRatio_Opn3km.d <- landscape_data$mnPerArRatio_Opn3km %>% (function(x) (x - mean(x, na.rm = T)) / sd(x, na.rm = T))

NNdist_Opn3km.d <- landscape_data$NNdist_Opn3km %>% (function(x) (x - mean(x, na.rm = T)) / sd(x, na.rm = T))
